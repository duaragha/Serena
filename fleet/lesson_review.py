...
def review_final_lessons(store, run_id: str, *, runner=None, capacity=None) -> None:
    ...
    if (
        ...
        or run["activity"] != "coding"
        or not all(leg["state"] == "completed" for phase in run["phases"] for leg in phase["legs"])
    ):
        return
    ...
    with store._connect() as db:
        ...
        if existing:
            ...
            if job["dispatches"] >= 2 or time.time() >= job["deadline"]:
                db.execute(
                    "UPDATE fleet_lesson_reviews SET state = 'expired', finished = ?, error = 'review recovery budget exhausted' WHERE id = ?",
                    (time.time(), job["id"]),
                )
                store._insert_event(
                    db,
                    run_id=run_id,
                    event_type="learning.review.expired",
                    payload={"review_id": job["id"]},
                )
                return
            db.execute(
                "UPDATE fleet_lesson_reviews SET dispatches = dispatches + 1, pid = NULL, process_token = NULL WHERE id = ?",
                (job["id"],),
            )
            ...
            candidates = [
                ...
                for row in db.execute(
                    ...
                    "WHERE l.source_run = ? AND l.state = 'candidate' AND a.state = 'completed' ORDER BY l.created",
                    (run_id,),
                )
            ]
            if not candidates:
            ...
            job = dict(
                ...
                provider=model_leg["runtime"],
                model=model_leg["model"],
                effort=model_leg["effort"],
            )
            db.execute(
                "INSERT INTO fleet_lesson_reviews(id,run_id,state,candidates,provider,model,effort,created,deadline,dispatches) "
                "VALUES (?,?,'running',?,?,?,?,?,?,1)",
                (
                    job["id"],
                    run_id,
        ...
        store._insert_event(
            db,
            run_id=run_id,
            event_type="learning.review.started",
            payload={
                "review_id": job["id"],
    ...
    def event(kind, payload):
        with store._connect() as db:
            if kind == "process.started":
                db.execute(
                    "UPDATE fleet_lesson_reviews SET pid = ?, process_token = ? WHERE id = ? AND state = 'running'",
                    (payload["pid"], process_start_token(payload["pid"]), job["id"]),
                )
            elif kind == "session.started":
                db.execute(
                    "UPDATE fleet_lesson_reviews SET session_id = ? WHERE id = ?",
    ...
    try:
        ...
        if (
            ...
            or available.get("usable") is not True
            or available.get("status") in {"unknown", "unavailable"}
        ):
            raise RuntimeError(
                "independent lesson review requires positive provider capacity; lessons remain unverified"
            )
        valid, changed = [], []
        ...
        if valid:
            request = WorkerRequest(
                run_id=run_id,
                leg_id=model_leg["leg_id"],
                attempt_id=job["id"],
                task=run["task"],
                activity=run["activity"],
                phase="verify",
                role="independent-lesson-reviewer",
                provider=job["provider"],
                model=job["model"],
                effort=job["effort"],
                access_mode="read",
                cwd=run["cwd"],
                worker_key="lesson-reviewer:" + job["id"],
                worker_label="Lesson reviewer",
                fleet_db_path=str(store.path),
                prompt=(
                    "Independently review these operational lesson candidates after Fix. You are a fresh "
            ...
            if (
                not expected_model_matches(job["provider"], job["model"], result.actual_model)
                or result.actual_effort != job["effort"]
            ):
                raise ValueError(
                    "lesson reviewer identity did not match its frozen Review model/effort"
                )
            blocks = re.findall(
                r"<serena-lesson-review>\s*(.*?)\s*</serena-lesson-review>",
                result.output_text,
                re.S,
            )
            if len(blocks) != 1:
            ...
            if (
                ...
                or len(decisions) != len(valid)
                or {d["id"] for d in decisions} != {c["id"] for c in valid}
            ):
                raise ValueError("lesson review must cover every candidate exactly once")
            if any(
                type(d["approve"]) is not bool
                or not isinstance(d["reason"], str)
                or not 10 <= len(d["reason"].strip()) <= 1000
                for d in decisions
            ):
        ...
        with store._connect() as db:
            ...
            for candidate in candidates:
                ...
                db.execute(
                    "UPDATE fleet_lessons SET state = ?, reviewer = ?, review_attempt = ?, review_reason = ? "
                    "WHERE id = ? AND state = 'candidate'",
                    (
                        "endorsed" if approved else "rejected",
                        "lesson-reviewer:" + job["id"],
                        job["id"],
                        redact_text(reason)[0],
                        candidate["id"],
                    ),
                )
                store._insert_event(
                    db,
                    run_id=run_id,
                    event_type="learning.reviewed",
                    payload={
                        "lesson_id": candidate["id"],
                        "state": "endorsed" if approved else "rejected",
                        "review_id": job["id"],
                    },
                )
            db.execute(
                "UPDATE fleet_lesson_reviews SET state = 'completed', finished = ? WHERE id = ?",
                (time.time(), job["id"]),
            )
            store._insert_event(
                db,
        ...
        with store._connect() as db:
            db.execute(
                "UPDATE fleet_lesson_reviews SET state = ?, finished = ?, error = ? WHERE id = ?",
                (state, time.time(), error, job["id"]),
            )
            store._insert_event(
                db,
                run_id=run_id,
                event_type="learning.review." + state,
                payload={"review_id": job["id"], "error": error},
            )
