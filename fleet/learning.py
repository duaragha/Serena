...
class FleetLearning:
    ...
    def propose(self, who: dict, summary: str, evidence_paths: list[str]) -> dict:
        ...
        with self.store._connect() as db:
            ...
            existing = db.execute(
                "SELECT * FROM fleet_lessons WHERE source_run = ? AND author = ? AND summary = ?",
                (who["run_id"], who["worker_key"], summary),
            ).fetchone()
            if existing:
    ...
    def review(self, who: dict, lesson_id: str, approve: bool, reason: str) -> dict:
        ...
        run = self.store.get_run(who["run_id"])
        leg = next(
            item for p in run["phases"] for item in p["legs"] if item["leg_id"] == who["leg_id"]
        )
        ...
        with self.store._connect() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
                "SELECT * FROM fleet_lessons WHERE id = ? AND source_run = ?",
                (lesson_id, who["run_id"]),
            ).fetchone()
            ...
            db.execute(
                "UPDATE fleet_lessons SET state = ?, reviewer = ?, review_attempt = ?, review_reason = ? WHERE id = ?",
    ...
    def retrieve(self, run: dict, attempt_id: str) -> list[dict]:
        ...
        with self.store._connect() as db:
            ...
            for row in rows:
                ...
                db.execute(
                    "INSERT OR IGNORE INTO fleet_lesson_uses(run_id,lesson_id,attempt_id) VALUES (?,?,?)",
    ...
    def finish(self, run: dict) -> None:
        ...
        with self.store._connect() as db:
            ...
            events = [
                json.loads(r[0])
                for r in db.execute(
                    "SELECT payload_json FROM fleet_events WHERE run_id = ? AND type = 'worker.integration.accepted'",
                    (run_id,),
                )
            ]
            gates = sum(
                bool(e.get("test_gate", {}).get("ran") and e.get("test_gate", {}).get("ok"))
                for e in events
            )
            attempts = db.execute(
                "SELECT COUNT(*) FROM fleet_attempts a JOIN fleet_legs l ON a.leg_id = l.leg_id WHERE l.run_id = ?",
                (run_id,),
            ).fetchone()[0]
            used = db.execute(
                "SELECT COUNT(DISTINCT lesson_id) FROM fleet_lesson_uses WHERE run_id = ?",
                (run_id,),
            ).fetchone()[0]
            total_legs = sum(len(p["legs"]) for p in run["phases"])
            duration = max(
                0, float(run.get("completed_at") or time.time()) - float(run["created_at"])
            )
            db.execute(
    ...
    def rollback(self, lesson_id: str, reason: str) -> None:
        ...
        with self.store._connect() as db:
            row = db.execute(
                "SELECT source_run FROM fleet_lessons WHERE id = ?", (lesson_id,)
            ).fetchone()
            if not row:
                raise KeyError(lesson_id)
            db.execute(
                "UPDATE fleet_lessons SET state = 'revoked', reason = ? WHERE id = ?",
                (redact_text(reason)[0][:1000], lesson_id),
            )
            self.store._insert_event(
                db, run_id=row[0], event_type="learning.revoked", payload={"lesson_id": lesson_id}
            )
        ...
        with self.store._connect() as db:
            return {
                "candidates": [
                    dict(r)
                    for r in db.execute(
                        "SELECT * FROM fleet_lessons WHERE source_run = ?", (run_id,)
                    )
                ],
                "uses": [
                    dict(r)
                    for r in db.execute(
                        "SELECT * FROM fleet_lesson_uses WHERE run_id = ?", (run_id,)
                    )
                ],
                "outcomes": [
                    dict(r)
