...
def main():
    with tempfile.TemporaryDirectory(prefix="serena-work-restart-proof-") as root:
        ...
        journal.claim_command("persisted-session", "work:accepted-item:dispatch", {"action": "work_submit"})
...
""", str(path)], cwd=Path(__file__).resolve().parents[1], text=True, capture_output=True, timeout=30)
        ...
        print("PASS: durable pending receipt preserved; child exited 0; no provider launched")
