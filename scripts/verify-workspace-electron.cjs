...
async function main() {
  ...
  try {
    ...
    await page.goto(`${base}/workspace/${sid}`);
    ...
    await page.getByRole('button', {name: 'Commands and skills', exact: true}).click();
    ...
    await page.getByRole('button', {name: 'Run shell command', exact: true}).click();
    const shell = page.getByRole('dialog', {name: 'Run shell command'});
    ...
    await page.locator('summary').filter({hasText: 'SERENA_ELECTRON_NATIVE'}).first().click();
    await page.getByText('SERENA_ELECTRON_NATIVE', {exact: true}).waitFor();
    await page.locator('.aw-state').filter({hasText:/^(ready|completed)$/}).waitFor();
    const input = page.getByRole('textbox', {name: 'Message Codex', exact: true});
    await app.evaluate(({clipboard}) => clipboard.writeText('first line\nsecond line'));
    await input.focus();
    await input.press('Control+V');
    assert.equal(await input.inputValue(), 'first line\nsecond line');
    await input.fill('');
    await page.getByText('SERENA_ELECTRON_NATIVE', {exact: true}).click({clickCount:3});
    ...
    assert.match(await page.locator('.aw-state').innerText(), /^(ready|completed)$/);
