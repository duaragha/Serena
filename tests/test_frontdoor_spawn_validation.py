...
def test_spawn_validation_accepts_posix_absolute_path():
...
def test_spawn_validation_rejects_duplicate_or_reordered_agents(monkeypatch):
    ...
    assert not _valid_spawn(
        {"agents": ["claude", "claude"], "cwd": "/home/raghav", "seed": "go"}
    )
    assert not _valid_spawn(
        {"agents": ["codex", "claude"], "cwd": "/home/raghav", "seed": "go"}
    )
