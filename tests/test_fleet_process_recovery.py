...
def test_killed_writer_resumes_with_its_patch_and_completed_research(fleet_env, monkeypatch):
    ...
    def command(request):
        ...
        if request.phase == "execute" and not killed:
            killed = True
            code += "Path('survived.txt').write_text('before death'); time.sleep(0.5); os.kill(os.getpid(), signal.SIGKILL)\n"
    ...
    assert parked["phases"][1]["legs"][0]["current_attempt"]["exit_code"] == -9
...
def test_process_retry_is_bounded_and_cancellation_never_wakes(tmp_path, cancelled):
    store = FleetStore(tmp_path / "fleet.sqlite3")
    run = _run(store)
    rid = run["run_id"]
    store.claim_run(rid)
    ...
    for index in range(3):
        ...
        store.finish_attempt(attempt["attempt_id"], state="cancelled" if cancelled else "failed",
                             exit_code=-9, error="worker exited with signal 9")
