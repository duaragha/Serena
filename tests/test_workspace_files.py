...
def test_git_lookup_respects_ignore_scope_spaces_and_environment(tmp_path, monkeypatch):
    ...
    (root / "selected file.py").write_text("private contents never returned", encoding="utf-8")
    (root / "ignored.py").write_text("ignored", encoding="utf-8")
    sibling = tmp_path / "elsewhere"
    sibling.mkdir()
    (sibling / "other.py").write_text("outside", encoding="utf-8")
...
def test_non_git_lookup_is_bounded_and_skips_generated_directories(tmp_path):
    ...
    (tmp_path / "node_modules" / "hidden.py").write_text("generated", encoding="utf-8")
