...
def create_tts_backend() -> AsyncTTSBackend:
    # config/voice-stack.json is the shared answer, so the desk and the phone
    # pick the same engine without either machine's unit file saying so.
    backend = voice_setting(
        "tts", "backend", env="SERENA_CALL_TTS_BACKEND", default="kokoro").lower()
...
class ElevenLabsTTSBackend(AsyncTTSBackend):
    ...
    def __init__(
        ...
        *,
        ...
        self.voice_id = str(
            voice_id
            or voice_setting("tts", "voice_id", env="SERENA_CALL_ELEVEN_VOICE_ID")
        ).strip()
        self.model_id = str(
            model_id
            or voice_setting(
                "tts", "model", env="SERENA_CALL_ELEVEN_MODEL",
