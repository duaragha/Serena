...
def test_activation_is_one_model_to_frozen_live_services(
    ...
    # One owner of the microphone: the wake listener, handing off to the orb.
    ...
    assert "serena-orb.service" in activation.UNITS
    stop_legacy = next(command for command in commands if "disable" in command)
    # The voice-only desk loop answered him with no orb; it never boots.
    assert "serena-desk.service" in stop_legacy
    assert "serena-brain-bridge.service" in stop_legacy
    assert "serena-wake-listener.service" not in stop_legacy
